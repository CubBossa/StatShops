...
public class BottomTopChestMenu extends ChestMenu {
	...
	public void loadInventory(Player player) {
		if (!inventories.containsKey(player)) {
			StatShops.getInstance().log(LoggingPolicy.WARN, "Could not restore inventory because no entry exists for " + player.getName());
			return;
		}
		for (int i = 0; i < bottomRows * ROW_SIZE; i++) {
	...
	public boolean handleAction(Player player, InventoryClickEvent event) {
		...
		if (clickHandler != null) {
			...
			try {
				...
				StatShops.getInstance().log(LoggingPolicy.ERROR, "Fehler bei handleInventoryClick() von Spieler " + player.getName(), exc);
			}
		}
	...
	public boolean closeInventory(Player player) {
		...
		try {
			...
			StatShops.getInstance().log(LoggingPolicy.ERROR, "Fehler bei handleInventoryClose() von Spieler " + player.getName(), exc);
		}
		return true;
	}

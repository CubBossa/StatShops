...
public class ListMenu<L extends ListMenuElement> extends PagedChestMenu {
	...
	public ListMenu(int rowCount, ListMenuElementHolder<L> elementHolder, boolean requireDeleteConfirmation,
	...
	private void prepareInventory() {
		...
		setNavigationEntry(7, ItemStackUtils.createItemStack(Material.EMERALD, newItemName, newItemLore), clickContext -> {
			...
			new AnvilGUI.Builder()
					...
					.onClose(p -> Bukkit.getScheduler().runTaskLater(ShopPlugin.getInstance(), () -> openInventory(player, currentPage), 1L))
					.onComplete((p, s) -> {

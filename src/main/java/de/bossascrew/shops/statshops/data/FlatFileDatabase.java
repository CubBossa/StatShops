...
public class FlatFileDatabase implements Database {
    ...
    public Map<UUID, Shop> loadShops() {
        ...
        for (File file : Arrays.stream(dirShops.listFiles()).filter(file -> file.getName().endsWith(".yml")).toList()) {
            try {
                UUID uuid;
                ...
                if(shopType == null) {
                    StatShops.getInstance().log(LoggingPolicy.ERROR, "Could not find ShopType '" + type + "' in shop: " + uuid);
                    continue;
                }

                Shop shop = shopType.supplyShop(uuid, nameFormat, dataMap);
                /*TODO switch (type) {
                ...
                StatShops.getInstance().log(LoggingPolicy.ERROR, "Could not load shop from file \"" + file.getName() + "\": " + t.getMessage());
            }
        }
    ...
    public void saveShop(Shop shop) {
        ...
        ConfigurationSection dataSlots = cfg.getConfigurationSection("data-slots");
        if (dataSlots == null) {
            dataSlots = cfg.createSection("data-slots");
        }
        for (Map.Entry<String, DataSlot<?>> dataSlot : shop.getData().entrySet()) {

...
public class ChestMenuShop extends BaseShop implements PaginatedShop {
    ...
    public boolean open(Customer customer, int page) {
        ...
        if (menuMap.containsKey(customer)) {
        ...
        Menu menu = newShopMenu(customer);
        menu.setPage(customer.getPlayer(), page);
        menu.open(customer.getPlayer());
        menu.setCloseHandler(c -> handleShopClose(customer)); //TODO cancelled behandeln

        menuMap.put(customer, (ShopMenu) menu);
    ...
    private boolean handleShopClose(Customer customer) {
        ShopCloseEvent event = new ShopCloseEvent(this, 0);
    ...
    public boolean close(Customer customer) {
        ...
        if (menu != null) {
            ...
            menu.close(customer.getPlayer());
            menuMap.remove(customer);
        }
        return handleShopClose(customer);
    }
